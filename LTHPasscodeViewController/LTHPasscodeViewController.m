...
#define PAD 40.0f
#define RADIUS 17.0f
#define Y_TEXTFIELD 155.0f

#define X_ENTER_PASSCODE_LABEL 40.0f
#define Y_ENTER_PASSCODE_LABEL 80.0f
#define HIGH_ENTER_PASSCODE_LABEL 40.0f

#define X_FAILED_ATTEMP_LABEL 20.0f
#define Y_FAILED_ATTEMP_LABEL 230.0f
#define HIGH_FAILED_ATTEMP_LABEL 40.0f
...
- (void)viewDidLoad {
    ...
    _label.font = appDelegate.largeFont;;
    _label.textAlignment = NSTextAlignmentCenter;
    _label.textColor = [UIColor blackColor];
    _label.text = @"fdsfsdf";
    // ^-Use UITextAlignmentCenter for older SDKs.
    _label.textColor = [UIColor whiteColor]; // change this color
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    //
    //     [self.navigationController.navigationBar setBackgroundImage:[UIImage  imageNamed:@"Slice_icon_112" ]forBarMetrics:UIBarMetricsDefault];
}
...
- (void)_setupLabels {
    ...
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")){
        ...
        _enterPasscodeLabel = [[UILabel alloc] initWithFrame: CGRectMake(X_ENTER_PASSCODE_LABEL, Y_ENTER_PASSCODE_LABEL, self.view.frame.size.width-X_ENTER_PASSCODE_LABEL*2, HIGH_ENTER_PASSCODE_LABEL)];
    ...
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")){
        _failedAttemptLabel = [[UILabel alloc] initWithFrame: CGRectMake(X_FAILED_ATTEMP_LABEL, Y_FAILED_ATTEMP_LABEL, self.view.frame.size.width-X_FAILED_ATTEMP_LABEL*2, HIGH_FAILED_ATTEMP_LABEL)];
...
- (void)_setupDigitFields {
    ...
    CGFloat  pad_trait = (screenWidth-3*PAD-4*RADIUS*2)/2;
    
    
    _firstDigitTextField = [self makeDigitFieldWithFrame:CGRectMake(pad_trait, Y_TEXTFIELD, RADIUS*2, RADIUS*2)];
    [_animatingView addSubview:_firstDigitTextField];
    
    
    _secondDigitTextField = [self makeDigitFieldWithFrame:CGRectMake(pad_trait+RADIUS*2+PAD, Y_TEXTFIELD, RADIUS*2, RADIUS*2)];
    
    [_animatingView addSubview:_secondDigitTextField];
    
    _thirdDigitTextField =  [self makeDigitFieldWithFrame:CGRectMake(pad_trait+2*RADIUS*2+PAD*2, Y_TEXTFIELD, RADIUS*2, RADIUS*2)];
    
    [_animatingView addSubview:_thirdDigitTextField];
    
    _fourthDigitTextField = [self makeDigitFieldWithFrame:CGRectMake(pad_trait+3*RADIUS*2+PAD*3, Y_TEXTFIELD, RADIUS*2, RADIUS*2)];
    
    [_animatingView addSubview:_fourthDigitTextField];
}

-(UITextField *)makeDigitFieldWithFrame:(CGRect)frame{
    UITextField *field = [[UITextField alloc] initWithFrame:frame];
    //    field.backgroundColor = _passcodeBackgroundColor;
    field.backgroundColor = [UIColor colorWithRed:69/255.0f green:164/255.0f blue:235/255.0f alpha:1];
    field.layer.cornerRadius = frame.size.width/2;
...
- (void)updateViewConstraints {
    ...
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")) {
        //
        //        CGFloat yOffsetFromCenter = -self.view.frame.size.height * 0.31;
        ...
        //        [enterPasscodeConstraintCenterX setActive:YES];
        //        [enterPasscodeConstraintCenterY setActive:YES];
        //        [self.view addConstraint: enterPasscodeConstraintCenterX];
        //        [self.view addConstraint: enterPasscodeConstraintCenterY];
        //        [_enterPasscodeLabel setTranslatesAutoresizingMaskIntoConstraints:YES];
...
- (void)showLockScreenWithAnimation:(BOOL)animated withLogout:(BOOL)hasLogout andLogoutTitle:(NSString*)logoutTitle {
    ...
    if (!_isCurrentlyOnScreen) {
        ...
        if (hasLogout) {
            ...
            // Navigation item
            BarButtonItem *leftButtonItem = [[BarButtonItem alloc]initWithButtonFrame:CGRectMake(0, 0, 45, 45)];
            [leftButtonItem setTitle:@"Done"];
            [leftButtonItem.button addTarget:self action:@selector(_logoutWasPressed) forControlEvents: UIControlEventTouchUpInside];
            
            UINavigationItem *item =
            [[UINavigationItem alloc] initWithTitle:self.label.text];
...
- (void)_prepareNavigationControllerWithController:(UIViewController *)viewController {
    ...
    BarButtonItem *cancelButtonItem = [[BarButtonItem alloc]initWithButtonFrame:CGRectMake(0, 0, 55, 45)];
    [cancelButtonItem setTitle:@"Cancel"];
    [cancelButtonItem setFontAndColorAndTextForButton:appDelegate.largeFont andColor:[UIColor whiteColor] andText:@"Cancel"];
    
    [cancelButtonItem.button addTarget:self action:@selector(_cancelAndDismissMe) forControlEvents: UIControlEventTouchUpInside];
...
- (void)showForEnablingPasscodeInViewController:(UIViewController *)viewController
    ...
    self.label.text = NSLocalizedStringFromTable(self.enablePasscodeString, _localizationTableName, @"");
...
- (void)showForChangingPasscodeInViewController:(UIViewController *)viewController
    ...
    [self _prepareNavigationControllerWithController:viewController];
    self.label.text = NSLocalizedStringFromTable(self.changePasscodeString, _localizationTableName, @"");
...
- (void)showForDisablingPasscodeInViewController:(UIViewController *)viewController
    ...
    [self _prepareNavigationControllerWithController:viewController];
    self.label.text = NSLocalizedStringFromTable(self.turnOffPasscodeString, _localizationTableName, @"");
...
- (void)_loadGapDefaults {
    ...
    _verticalGap = UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ? 60.0f : 50.0f;
