...
//@property (nonatomic, strong) dispatch_queue_t uploadLocalDocumentToCloudQueue;
//@property (nonatomic, strong) dispatch_queue_t deleteDocumentQueue;
...
//        _uploadLocalDocumentToCloudQueue = dispatch_queue_create("uploadLocalDocumentToCloud", NULL);
//        _deleteDocumentQueue = dispatch_queue_create("deleteDocumentQueue", NULL);
        _syncQueue = dispatch_queue_create("syncQueueIcloud", NULL);
        _alert = [[UIAlertView alloc]initWithTitle:@"" message:@"" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        
        self.stopRunning = NO;
        _isSyncing = NO;
        _suspendedWhileSyncing = NO;
...
- (void)uploadLocalDocumentToCloudWithName:(NSString *)documentName localPath:(NSString*)local_path completion:(void (^)(NSError *error))handler {
    // Log download
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0ul), ^{
        ...
        //Delete all file in temp
            ...
            for (NSString *file in [fileManager contentsOfDirectoryAtPath:NSTemporaryDirectory() error:&error])
            {
        ...
        NSLog(@"icloudThread:CurrentTHread: %@ :MainThread :%@",[NSThread currentThread],[NSThread mainThread]);
        // Get the array of files in the documents directory
        NSString *documentsDirectory = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
        NSString *localDocument = [[NSString stringWithFormat:@"%@/%@",documentsDirectory,local_path] stringByAppendingPathComponent:documentName];
        ...
        if (![self.previousQueryResults containsObject:localDocument]) {
            ...
            @autoreleasepool {
                ...
                NSString *tempPath = [NSTemporaryDirectory() stringByAppendingPathComponent:localURL.lastPathComponent];
                
                NSError *attributeFileError;
                
                [[NSFileManager  defaultManager]createFileAtPath:tempPath contents:[NSData dataWithContentsOfURL:localURL] attributes:[[NSFileManager defaultManager]attributesOfItemAtPath:localURL.path error:&attributeFileError]];
                
                NSLog(@"attribute:%@:%@",[[NSFileManager defaultManager]attributesOfItemAtPath:tempPath error:&attributeFileError],[[NSFileManager defaultManager]attributesOfItemAtPath:localURL.path error:&attributeFileError]);
                
                NSError *error;
                NSError *errorDeleteTempFile;
                
                BOOL success = [self.fileManager setUbiquitous
                                :YES itemAtURL:[NSURL fileURLWithPath:tempPath] destinationURL:cloudURL error:&error];
                
                [[NSFileManager defaultManager]removeItemAtPath:tempPath error:&errorDeleteTempFile];
                ...
                if (!success) {
                    ...
                    NSError *errorGetAttribute;
                    
                    NSLog(@"%@",[_fileManager attributesOfItemAtPath:[cloudURL path]error:&errorGetAttribute]);
                    dispatch_async(dispatch_get_main_queue(), ^{
                        handler(nil);
                        return;
                    });
