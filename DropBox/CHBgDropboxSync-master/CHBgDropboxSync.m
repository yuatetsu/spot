...
@interface CHBgDropboxSync() {
    ...
    BOOL imageSyncFinish;
    BOOL memoVoiceSyncFinish;
    BOOL spotDataSyncFinish;
    BOOL guideBookDataSyncFinish;
    BOOL fileExistOnLocal;
    BOOL fileExistOnDropbox;
    int deleteCount;
}
...
- (void)startup {
        ...
        guideBookDataSyncFinish = NO;
        imageSyncFinish = NO;
        memoVoiceSyncFinish = NO;
        spotDataSyncFinish = NO;
        fileExistOnLocal = NO;
        fileExistOnDropbox = NO;
        deleteCount = 0;
        
        [[NSNotificationCenter defaultCenter]postNotificationName:@"CHBgDropboxSyncing" object:nil];
        ...
        [client loadMetadata :$str(@"/%@",IMAGE_PATH)];
...
- (void)internalCommonShutdown {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"CHBgDropboxSyncshutdown" object:nil];
...
- (void)stepComplete {
    ...
    NSString *pathFolder;
    if (imageSyncFinish ==NO) {
        
        pathFolder = IMAGE_PATH;
    }
    
    else if (memoVoiceSyncFinish == NO){
        
        pathFolder = MEMO_VOICE_DATA_PATH;
    }
    
    else if (spotDataSyncFinish == NO){
        
        pathFolder = SPOT_DATA_PATH;
    }
    else if (guideBookDataSyncFinish == NO){
        
        pathFolder = GUIDE_BOOK_DATA_PATH;
    }
    [client performSelector:@selector(loadMetadata:) withObject:$str(@"/%@",pathFolder) afterDelay:.05];
}
...
// Upload
- (void)startTaskUpload:(NSString*)file rev:(NSString*)rev toPath:(NSString*)path {
    NSLog(@"Sync: Uploading file %@, %@", file, rev?@"overwriting":@"new");
    [client uploadFile:file toPath:[NSString stringWithFormat:@"/%@",path] withParentRev:rev fromPath:[[NSString stringWithFormat:@"%@/%@",$.documentPath,path] stringByAppendingPathComponent:file]];
}
...
- (void)restClient:(DBRestClient *)client uploadedFile:(NSString *)destPath from:(NSString *)srcPath metadata:(DBMetadata *)metadata {
    ...
    [data setObject:destPath.lastPathComponent forKey:@"file_name"];
    [data setObject:metadata.lastModifiedDate forKey:@"date_dropbox"];
    
    NSError *error;
    NSDictionary *attribute = [[NSFileManager defaultManager]attributesOfItemAtPath:srcPath error:&error];
    if (!error) {
        
        [data setObject:attribute.fileModificationDate forKey:@"date_local"];
    }
    else{
        
        [data setObject:[NSDate date] forKey:@"date_local"];
    }
    
    if (fileExistOnDropbox) {
        
        [self saveDropboxInfoToDatabase:data existFile:YES];
    }
    else{
        
        [self saveDropboxInfoToDatabase:data existFile:NO];
    }
...
// Download
- (void)startTaskDownload:(NSString*)file localPath:(NSString*)path{
    NSLog(@"Sync: Downloading file %@", file);
    NSLog(@"%@",[$.documentPath stringByAppendingPathComponent:$str(@"%@/%@",path,file)]);
    [client loadFile:$str(@"/%@/%@",path,file) intoPath:[$.documentPath stringByAppendingPathComponent:$str(@"%@/%@",path,file)]];
}
...
- (void)restClient:(DBRestClient*)client loadedFile:(NSString*)destPath contentType:(NSString*)contentType metadata:(DBMetadata*)metadata {
    ...
    else if (!memoVoiceSyncFinish){
        
    }
    else if(!spotDataSyncFinish){
        
        [(AppDelegate*)[UIApplication sharedApplication].delegate parseXMLSyncDataToSpotObject:[[NSURL alloc]initFileURLWithPath:destPath] newEntity:!fileExistOnLocal];
    }
    else if(!guideBookDataSyncFinish){
        
        [(AppDelegate*)[UIApplication sharedApplication].delegate parseXMLSyncDataToGuideBookObject:[[NSURL alloc]initFileURLWithPath:destPath] newEntity:!fileExistOnLocal];
    }
    ...
    NSError *error;
    NSDictionary *attribute = [[NSFileManager defaultManager]attributesOfItemAtPath:destPath error:&error];
    if (!error) {
...
- (void)restClient:(DBRestClient *)client deletedPath:(NSString *)path {
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:@"FinishDeletedFileonDropbox" object:nil userInfo:@{FILE_PATH_DELETE_DROPBOX_KEY:path.lastPathComponent}];
    
    NSLog(@"DeleteFileFinish");
    ...
    if (deleteCount==0)
    {
        [[NSNotificationCenter defaultCenter]postNotificationName:@"FinishDeletedFilesonDropbox" object:nil];
    }
}
...
- (void)restClient:(DBRestClient *)client deletePathFailedWithError:(NSError *)error {
    ...
    {
       [[NSNotificationCenter defaultCenter]postNotificationName:@"FinishDeletedFileds" object:nil];
    }
    
    [[NSNotificationCenter defaultCenter] postNotificationName:@"CHBgDropboxDeleteFailed" object:nil];
    
    __autoreleasing DBRestClient* autoreleaseClient = self->client;
    [autoreleaseClient description];
...
- (NSDictionary*)getLocalStatus {
    ...
    for (NSString* item in [[NSFileManager defaultManager] contentsOfDirectoryAtPath:[NSString stringWithFormat:@"%@/%@",root,pathFolder] error:nil]) {
        ...
        NSString* itemPath = [[NSString stringWithFormat:@"%@/%@",root,pathFolder] stringByAppendingPathComponent:item];
...
- (BOOL)syncStepWithRemoteFiles:(NSDictionary*)remoteFiles andRevs:(NSDictionary*)remoteRevs {
        ...
        for (NSString* file in all) {
            ...
            if (local && remote) {
                ...
                NSMutableDictionary *data = [self getInfoDropboxFile:file];
                NSDate  *date_dropbox=nil;
                NSDate  *date_local=nil;
                if (data[@"date_dropbox"]) {
                    
                    date_dropbox =data[@"date_dropbox"];
                }
                if(data[@"date_local"]){
                ...
                BOOL same;
                if (date_local&&date_dropbox) {
                    // File is in both places, but are the dates the same?
                    double delta = date_dropbox.timeIntervalSinceReferenceDate - remote.timeIntervalSinceReferenceDate;
                    same = ABS(delta)<0.5; // If they're within 2 seconds, that's close enough to be the same
                }
                else {
                ...
                if (!same) {
                    ...
                    if (date_dropbox.timeIntervalSinceReferenceDate > remote.timeIntervalSinceReferenceDate) {
                        // Local is newer
                        // So send the local file to dropbox, overwriting the existing one with the given 'rev'
                        fileExistOnDropbox = YES;
                        [self startTaskUpload:file rev:[remoteRevs objectForKey:file] toPath:pathFolder];
                        return NO;
                    } else {
                        // Remote is newer
                        // So download the file
                        fileExistOnLocal = YES;
                        [self startTaskDownload:file localPath:pathFolder ];
                        return NO;
                    }
                }
                else{
                    ...
                    if(date_local){
                        ...
                        double delta_local = local.timeIntervalSinceReferenceDate - date_local.timeIntervalSinceReferenceDate;
                        BOOL same_local = ABS(delta_local)<0.5;
                        
                        if (!same_local) {
                            
                            if (local.timeIntervalSinceReferenceDate > date_local.timeIntervalSinceReferenceDate) {
                                // Local is newer
                                // So send the local file to dropbox, overwriting the existing one with the given 'rev'
                                fileExistOnDropbox = YES;
                                [self startTaskUpload:file rev:[remoteRevs objectForKey:file] toPath:pathFolder];
        ...
        if (imageSyncFinish ==NO) {
            ...
            memoVoiceSyncFinish =YES;
            [self stepComplete];
        }
    
        else if (spotDataSyncFinish == NO){
            
            spotDataSyncFinish =YES;
            [self stepComplete];
        }
        else if (guideBookDataSyncFinish == NO){
            
            guideBookDataSyncFinish=YES;
            [self stepComplete];
        }
        
        else{
            NSLog(@"CurrentThread:%@  MainThread:%@",[NSThread currentThread],[NSThread mainThread]);
            [[NSNotificationCenter defaultCenter]postNotificationName:@"CompleteSyncDropbox" object:nil];
...
- (void)restClient:(DBRestClient*)_client loadedMetadata:(DBMetadata*)metadata {
    ...
    for (DBMetadata* item in metadata.contents) {
        if (item.isDirectory) {
            ...
            [remoteFiles setObject:item.lastModifiedDate forKey:[self noSlash:item.filename.lastPathComponent]];
            [remoteFileRevs setObject:item.rev forKey:[self noSlash:item.path.lastPathComponent]];
        }
    }
...
+(void)deleteFileAtPath:(NSString *)filePath{
    ...
    [[self i]deleteFileFromDropbox:filePath];
}

-(void)deleteFileFromDropbox:(NSString *)filePath{
    ...
    [client deletePath:filePath];
}

+(void)increaseDeleteCount:(int)count{
    
    [self i]->deleteCount += count;
}

#pragma mark - SaveDropboxFileInfoToDatabase

-(void)saveDropboxInfoToDatabase:(NSMutableDictionary*)info existFile:(BOOL)exist{
    
    [(AppDelegate*)[UIApplication sharedApplication].delegate updateDropboxModifiedDateDataDatabaseforNewEntity:!exist data:info];
}

-(NSMutableDictionary *)getInfoDropboxFile:(NSString*)fileName{
    
    return  [(AppDelegate*)[UIApplication sharedApplication].delegate getModifiedDateDropboxFromFilename:fileName];
}
@end
